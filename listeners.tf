...
resource "aws_lb_listener_rule" "http_tcp_listener_rule" {
  count = length(var.http_listener_rules) > 0 ? length(var.http_listener_rules) : 0

  listener_arn = lookup(var.http_listener_rules[count.index], "listener_arn", null)
  ...
  tags = merge(
    var.tags,
    lookup(var.http_listener_rules[count.index], "tags", {}),
  )
}

resource "aws_lb_listener_certificate" "this" {
  count           = length(var.https_listener_certificates) > 0 ? length(var.https_listener_certificates) : 0
  listener_arn    = var.https_listener_certificates[count.index]["listener_arn"]
  certificate_arn = var.https_listener_certificates[count.index]["certificate_arn"]
}
