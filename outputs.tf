...
output "service_role_arn" {
  ...
  value       = try(aws_iam_role.ecs_service[0].arn, "")
}
...
output "service_security_group_id" {
  ...
  value       = try(aws_security_group.ecs_service[0].id, "")
}

output "task_definition_family" {
  description = "ECS task definition family"
  value       = try(aws_ecs_task_definition.this[0].family, "")
}
...
output "alb_target_group_arn" {
  description = "ECS task definition family"
  value       = try(aws_alb_target_group.this[0].arn, "")
}

output "scale_up_policy_arn" {
  description = "ARN of the scale up policy"
  value       = try(aws_appautoscaling_policy.up[0].arn, "")
}

output "scale_down_policy_arn" {
  description = "ARN of the scale down policy"
  value       = try(aws_appautoscaling_policy.down[0].arn, "")
}
